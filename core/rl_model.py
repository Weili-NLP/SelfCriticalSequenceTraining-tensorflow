...
class CaptionGenerator(object):
    ...
    def build_loss(self):
        ...
        for t in range(self.T):
            ...
            loss.append( tf.transpose(tf.mul(tf.transpose(tf.log(tf.clip_by_value(softmax, 1e-20, 1.0)) * tf.one_hot(captions[:, t], 23111), [1, 0]),  mask[:, t]), [1, 0]))
    ...
    def build_multinomial_sampler(self, max_len=16):
        ...
        for t in range(self.T):
            ...
            loss.append(tf.log(tf.clip_by_value(softmax, 1e-20, 1.0)) * tf.one_hot(tf.identity(sampled_word), 23111))
